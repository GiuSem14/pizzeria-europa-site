...
// runtime, quindi le varianti vanno scritte per esteso. Serve solo al ramo
// multi-sede: con una sola sede attiva il layout è card + mappa affiancate.
const GRID_SEDI = {
...
// Scheda informativa di una sede (nome, indirizzo, telefono, WhatsApp).
const SedeCard = ({ sede }) => (
  <div className="bg-cream-light rounded-2xl p-7">
    <h3 className="font-heading text-xl text-ink font-semibold mb-1">{sede.nome}</h3>
    <p className="font-body text-sm text-ink-muted mb-1">{sede.indirizzo}</p>
    <p className="font-body text-sm text-ink-muted mb-4">{sede.citta}</p>
    <a
      href={telHref(sede.telefono)}
    ...
    <>
      ...
      <section className="bg-cream py-20">
        <div className="max-w-6xl mx-auto px-6">
          ...
          {sedeUnica ? (
            // Una sola sede: card e mappa affiancate su desktop, impilate su
            // mobile. `items-stretch` (default della griglia) tiene le due
            // colonne alla stessa altezza.
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <SedeCard sede={sedeUnica} />
              <MappaSede
                sede={sedeUnica}
                height="100%"
