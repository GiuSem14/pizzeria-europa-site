    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-cream-light shadow-sm">
      <div className="max-w-6xl mx-auto px-6 py-2 flex items-center">
        ...
        <div className="flex-1">
          ...
          >
            <img src={logo} alt="Logo Pizzeria Europa" className="h-12 w-auto object-contain" />
          </Link>
        </div>
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                `font-body text-sm font-medium transition-colors ${
                  isActive ? 'text-tomato' : 'text-ink hover:text-tomato'
                }`
              }
            >
              {label}
      ...
      {/* Mobile drawer */}
      {open && (
        <div className="md:hidden bg-cream-light border-t border-cream shadow-lg">
          <nav className="flex flex-col px-6 py-4 gap-1">
            {links.map(({ to, label }) => (
              <NavLink
                ...
                className={({ isActive }) =>
                  `font-body text-base font-medium py-3 border-b border-cream transition-colors ${
                    isActive ? 'text-tomato' : 'text-ink hover:text-tomato'
                  }`
                }
                onClick={() => setOpen(false)}
